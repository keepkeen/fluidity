...
const StyledStartpage = styled.div<{
  cardHidden: boolean
  layoutMode: "carousel" | "dashboard"
}>`
  padding: 0px 100px;
  display: flex;
  flex-direction: row;
  justify-content: ${({ cardHidden, layoutMode }) =>
    cardHidden || layoutMode === "dashboard" ? "center" : "flex-start"};
  align-items: ${({ layoutMode }) =>
    layoutMode === "dashboard" ? "flex-start" : "center"};
  flex: 1;
  min-height: 0;
  gap: 32px;
...
export const Startpage = () => {
  ...
  const layoutMode = cardAreaSettings.layoutMode || "carousel"
  const isDashboard = layoutMode === "dashboard" && !cardHidden
  ...
  return (
    <>
      ...
      <Wrapper>
        ...
        <StyledStartpage cardHidden={cardHidden} layoutMode={layoutMode}>
          {/* 卡片区域 */}
          {/* Dashboard Mode */}
          {isDashboard && (
            <Suspense fallback={null}>
          ...
          {/* Carousel Mode */}
          {!isDashboard && !cardHidden && carouselItems.length > 0 && (
            <CarouselContainer
