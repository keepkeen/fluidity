...
const StyledAccordionContainer = styled.div<{ compact?: boolean }>`
  ...
  @media screen and (max-width: 600px) {
    ...
    flex-wrap: nowrap;
    gap: 10px;
    padding-top: ${({ compact }) => (compact ? "44px" : "0")};
  }
`
...
const StyledAccordionGroup = styled.div<{
  ...
  @media screen and (max-width: 600px) {
    width: 100%;
    height: ${({ active, dynamicHeight, compact }) =>
      active
        ? `${Math.round(dynamicHeight * (compact ? 0.8 : 0.7))}px`
        : "72px"};
    margin-bottom: 0;
...
const AccordionContent = styled.div<{ width: number }>`
  ...
  @media screen and (max-width: 600px) {
    width: ${({ width }) => (width > 0 ? "calc(100% - 66px)" : "0px")};
    flex: ${({ width }) => (width > 0 ? "1 1 auto" : "0 0 0")};
  }
`
